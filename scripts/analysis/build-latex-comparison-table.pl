...
# Get the data set for the table
...
sub loadDataFile
{
    ...
    while( <DATA> )
    {
        ...
        # Does the key match our dataset?
        if( $key =~ /$dataSet/ )
        {
            ...
            if( $key =~ /summary/ )
            {
                ...
                my ($trash, $personality, $indCount, $dataSetStr, $type) =
                        split( /\./, $key );
                $personality =~ s/personality-0(\d)/0\.\1/;
                $indCount =~ s/indcount-//;

#print "indCount=[$indCount] personality=[$personality] type=[$type] value=[$value]\n";
            ...
            elsif( $key =~ /t-test.*p-value/ )
            {
                ...
                $key =~ /personality-(0\.\d)-indCount-(\d+).*personality-(0\.\d)-indCount-(\d+)/;
