...
$changepointFiles[0] =~ /indcount-(\d+)/;
my $formattedIndCount = $1;

# -------------------------------------------------------------------
# Get the emergence times from each changepoint analysis
my %data;
...
sub readEmergenceTimes
{
    ...
    # ---------------------------------------------------------------
    # Open the file
    open( DATA, "$changePointFile" ) or die "Unable to open changepoint file [$changePointFile]: $!\n";
    ...
    while( <DATA> )
    {
        ...
        # Is it the changepoints?
        if( $dataType =~ /changepoints/ )
        {
            # Yup
            @{$rawData{$ind}{"changepoints"}} = split( /\s+/, $value );
        }
...
#print "Ind=[$ind] coefficient=[",${$rawDataRef->{$ind}{"coefficients"}}[$i],"] i=[$i]\n";
                ...
                # start at timestep 0, so the changepoint indices
                ...
                else
                {
                    push( @rawEmergenceTimes,
                            ${$rawDataRef->{$ind}{"changepoints"}}[$i-1] );
                }
