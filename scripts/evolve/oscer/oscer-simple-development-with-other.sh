...
DATADIR="data/simple-dev-with-$OTHERPART-predation-$THISPART"
mkdir $DATADIR

./evolve.sh \
    --params evolve/experiments/simple-development-with-other.parameters \
    --logprefix log \
    --jargs -Dactivation-function-mapping-file=./cfg/activation-functions.properties \
    --runid simple-dev-with-$OTHERPART-predation-$THISPART-experiment-$SEED \
    --statfile $DATADIR/simple-dev-with-$OTHERPART-predation-$OTHERPART-$SEED.stats \
    --jobcount $JOBS \
    ...
    --seed $SEED > simple-dev-with-$OTHERPART-predation-$THISPART-$SEED.out 2>&1
