...
DATADIR="data/simple-dev-only-$OTHERPART-predation-$THISPART"
mkdir $DATADIR

./evolve.sh \
    --params evolve/experiments/simple-development-only-other.parameters \
    --logprefix log \
    --jargs -Dactivation-function-mapping-file=./cfg/activation-functions.properties \
    --runid simple-dev-only-$OTHERPART-predation-$THISPART-experiment-$SEED \
    --statfile $DATADIR/simple-dev-only-other-$OTHERPART-predation-$OTHERPART-$SEED.stats \
    --jobcount $JOBS \
    ...
    --seed $SEED > tmp/simple-dev-only-$OTHERPART-predation-$THISPART-$SEED.out 2>&1
